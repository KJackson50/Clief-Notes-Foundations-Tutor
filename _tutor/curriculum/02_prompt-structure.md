...
A framework for building prompts that get useful results on the first try. Five parts. Works for writing, coding, research, or anything else you throw at Codex.



The Lesson

A prompt is an instruction set. The clearer the instruction, the better the result. Most people write prompts the way they send a text message. That works for simple questions. It falls apart the moment you want Codex to do real work.

If you went through Section 1.2: Your First Folder, you already know that giving Codex structured context changes the output. The folder handles the big picture: who you are, what the project is, what good looks like. This lesson handles the individual ask. Each time you need Codex to do something specific, how do you phrase it so you get something useful back?
...
Part 1: Identity — Who is Codex right now?

Tell Codex what role to fill. This shapes the vocabulary, the depth, and the assumptions it makes.
...
If you set up an AGENTS.md file in 1.2, this part is already handled for your project. The file does this work for you in every conversation. But for one-off tasks or tasks where you need Codex to shift roles, you add identity at the top of your prompt.

One note: identity is the part most people skip. They jump straight to "write me a blog post" without telling Codex who is writing it. The output is always more generic without it.



Part 2: Task — What needs to get done?

Be specific. "Help me write something" is vague. "Write a 200-word product description for [X] targeting [Y] audience" gives Codex something to work with.
...
Part 3: Context — What does Codex need to know?

This is the background. The constraints. The audience. Prior decisions. Relevant data. Anything Codex needs to do the task well that it would not know on its own.
...
The more relevant context you give, the less Codex has to guess. Guessing is where AI outputs go sideways. If the output feels generic or off-target, the fix is almost always more context, not a better prompt.



Part 4: Constraints — What should Codex avoid?

This is the part where most people leave value on the table. Telling Codex what you do NOT want is just as useful as telling it what you do want.
...
Constraints save you editing time. Every constraint you give is a mistake Codex will not make. Think about the last three times an AI output annoyed you. Those annoyances are constraints you did not set.



Part 5: Output Format — What should the result look like?

Tell Codex the shape of the answer. A list? A table? Three options to choose from? A code block with comments? A draft with placeholder sections?
...
Codex will try to do all of it. The output will be shallow across the board because it is spreading its effort too thin.
...
Each step builds on the last. You review and correct between steps. Codex gets cleaner input each time because you narrowed the scope. And if something goes wrong at step 3, you only redo step 3, not the whole thing.
...
- "Let's start with Identity. What role should Codex be in for this task? Write that line."
...
- "Now Context. What background does Codex need that it wouldn't know on its own?"
  - [User writes it.]
- "Now Constraints. What do you NOT want Codex to do? What would annoy you if it showed up in the output?"
  - [User writes it.]
...
2. Ask: "The next time you get a vague or unhelpful response from Codex — what's the first thing you'd check in your prompt?"

3. Record in `_tutor/progress.md`:
   - lesson: "02_prompt-structure"
   - artifacts_inspected: ["[path to their prompt file]"]
   - comprehension.question: "The next time you get a vague or unhelpful response from Codex — what's the first thing you'd check in your prompt?"
